/**********************************************************************************************
 * 4wings - Version 0.1
 * by Massimiliano Mosca
 *
 * This software is licensed under a GPLv3 License
 **********************************************************************************************/

#include "pid.h"
#include "I2Cdev.h"
#include "MPU6050_9Axis_MotionApps41.h"

#if I2CDEV_IMPLEMENTATION == I2CDEV_ARDUINO_WIRE
    #include "Wire.h"
#endif

#define FAST_INVERSE_SQRT

#define LED_PIN 13
bool blinkState = false;

MPU6050 mpu;
// global constants for 9 DoF fusion and AHRS (Attitude and Heading Reference System)
#define GyroMeasError PI * (40.0f / 180.0f)       // gyroscope measurement error in rads/s (shown as 3 deg/s)
#define GyroMeasDrift PI * (0.0f / 180.0f)      // gyroscope measurement drift in rad/s/s (shown as 0.0 deg/s/s)
#define beta sqrt(3.0f / 4.0f) * GyroMeasError   // compute beta
#define zeta sqrt(3.0f / 4.0f) * GyroMeasDrift   // compute zeta, the other free parameter in the Madgwick scheme usually set to a small or zero value

float q[4] = {1.0f, 0.0f, 0.0f, 0.0f};    // vector to hold quaternion
int16_t ax, ay, az;
int16_t gx, gy, gz;
int16_t mx, my, mz;
float pitch, yaw, roll;
unsigned long deltaTime;
unsigned long counter=0;
String outputString; 

void setup() {
	// join I2C bus (I2Cdev library doesn't do this automatically)
    #if I2CDEV_IMPLEMENTATION == I2CDEV_ARDUINO_WIRE
        Wire.begin();
        TWBR = 12;  //Override the scaler to run I2C @ 400Hz on 16MHz Arduino (prescaler = 1)
    #elif I2CDEV_IMPLEMENTATION == I2CDEV_BUILTIN_FASTWIRE
        Fastwire::setup(400, true);
    #endif

    Serial.println("Initializing I2C IMU devices...");
    mpu.initialize();

    Serial.println("Testing device connections...");
    Serial.println(mpu.testConnection() ? "MPU9150 connection successful" : "MPU9150 connection failed");
    mpu.setRate(7);
    mpu.setDLPFMode(1);
    mpu.setFullScaleGyroRange(0); // set gyro range to 250 degrees/sec
	mpu.setFullScaleAccelRange(0); // set accelerometer to 2 g range
	mpu.setIntDataReadyEnabled(true);

    pinMode(LED_PIN, OUTPUT);
	Serial.println(F("counter, time, deltaTime, accel [x,y,z], gyro [x,y,z], magn [x,y,z], attitude [yaw,pitch,roll]:"));

}

void loop() {
	if(mpu.getIntDataReadyStatus() == 1) { 
        unsigned long startTime = micros();
        mpu.getAcceleration( &ax, &ay, &az );
        mpu.getRotation( &gx, &gy, &gz );
        mpu.getMag( &mx, &my, &mz );

        MadgwickQuaternionUpdate(ax,ay,az,gx,gy,gz,mx,my,mz);
		
	    yaw   = atan2(2.0f * (q[1] * q[2] + q[0] * q[3]), q[0] * q[0] + q[1] * q[1] - q[2] * q[2] - q[3] * q[3]);   
	    pitch = -asin(2.0f * (q[1] * q[3] - q[0] * q[2]));
	    roll  = atan2(2.0f * (q[0] * q[1] + q[2] * q[3]), q[0] * q[0] - q[1] * q[1] - q[2] * q[2] + q[3] * q[3]);
	    pitch *= 180.0f / PI;
	    yaw   *= 180.0f / PI - 13.8; // Declination at Danville, California is 13 degrees 48 minutes and 47 seconds on 2014-04-04
	    roll  *= 180.0f / PI;

        deltaTime = micros() - startTime;
        outputString =  String(counter) + "," + String(startTime) + "," + String(deltaTime) + ",";
        outputString += String(ax) + "," + String(ay) + "," + String(az) + ",";
        outputString += String(gx) + "," + String(gy) + "," + String(gz) + ",";
        outputString += String(mx) + "," + String(my) + "," + String(mz) + ",";
        outputString += String(yaw) + "," + String(pitch) + "," + String(roll);
        Serial.println(outputString);
        counter++;
    }
}


void MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
        {
            float q1 = q[0], q2 = q[1], q3 = q[2], q4 = q[3];   // short name local variable for readability
            float norm;
            float hx, hy, _2bx, _2bz;
            float s1, s2, s3, s4;
            float qDot1, qDot2, qDot3, qDot4;

            // Auxiliary variables to avoid repeated arithmetic
            float _2q1mx;
            float _2q1my;
            float _2q1mz;
            float _2q2mx;
            float _4bx;
            float _4bz;
            float _2q1 = 2.0f * q1;
            float _2q2 = 2.0f * q2;
            float _2q3 = 2.0f * q3;
            float _2q4 = 2.0f * q4;
            float _2q1q3 = 2.0f * q1 * q3;
            float _2q3q4 = 2.0f * q3 * q4;
            float q1q1 = q1 * q1;
            float q1q2 = q1 * q2;
            float q1q3 = q1 * q3;
            float q1q4 = q1 * q4;
            float q2q2 = q2 * q2;
            float q2q3 = q2 * q3;
            float q2q4 = q2 * q4;
            float q3q3 = q3 * q3;
            float q3q4 = q3 * q4;
            float q4q4 = q4 * q4;

            // Normalise accelerometer measurement
            #ifdef FAST_INVERSE_SQRT
            	norm = fastInvSqrt(ax * ax + ay * ay + az * az);
            #else
            	norm = sqrt(ax * ax + ay * ay + az * az);
            	if (norm == 0.0f) return; // handle NaN
            	norm = 1.0f/norm;
            #endif
            ax *= norm;
            ay *= norm;
            az *= norm;

            // Normalise magnetometer measurement
            #ifdef FAST_INVERSE_SQRT
            	norm = fastInvSqrt(mx * mx + my * my + mz * mz);
            #else
            	norm = sqrt(mx * mx + my * my + mz * mz);
            	if (norm == 0.0f) return; // handle NaN
            	norm = 1.0f/norm;
            #endif            
            mx *= norm;
            my *= norm;
            mz *= norm;

            // Reference direction of Earth's magnetic field
            _2q1mx = _2q1 * mx;
            _2q1my = _2q1 * my;
            _2q1mz = _2q1 * mz;
            _2q2mx = _2q2 * mx;
            hx = mx * q1q1 - _2q1my * q4 + _2q1mz * q3 + mx * q2q2 + _2q2 * my * q3 + _2q2 * mz * q4 - mx * q3q3 - mx * q4q4;
            hy = _2q1mx * q4 + my * q1q1 - _2q1mz * q2 + _2q2mx * q3 - my * q2q2 + my * q3q3 + _2q3 * mz * q4 - my * q4q4;
            _2bx = sqrt(hx * hx + hy * hy);
            _2bz = -_2q1mx * q3 + _2q1my * q2 + mz * q1q1 + _2q2mx * q4 - mz * q2q2 + _2q3 * my * q4 - mz * q3q3 + mz * q4q4;
            _4bx = 2.0f * _2bx;
            _4bz = 2.0f * _2bz;

            // Gradient decent algorithm corrective step
            s1 = -_2q3 * (2.0f * q2q4 - _2q1q3 - ax) + _2q2 * (2.0f * q1q2 + _2q3q4 - ay) - _2bz * q3 * (_2bx * (0.5f - q3q3 - q4q4) + _2bz * (q2q4 - q1q3) - mx) + (-_2bx * q4 + _2bz * q2) * (_2bx * (q2q3 - q1q4) + _2bz * (q1q2 + q3q4) - my) + _2bx * q3 * (_2bx * (q1q3 + q2q4) + _2bz * (0.5f - q2q2 - q3q3) - mz);
            s2 = _2q4 * (2.0f * q2q4 - _2q1q3 - ax) + _2q1 * (2.0f * q1q2 + _2q3q4 - ay) - 4.0f * q2 * (1.0f - 2.0f * q2q2 - 2.0f * q3q3 - az) + _2bz * q4 * (_2bx * (0.5f - q3q3 - q4q4) + _2bz * (q2q4 - q1q3) - mx) + (_2bx * q3 + _2bz * q1) * (_2bx * (q2q3 - q1q4) + _2bz * (q1q2 + q3q4) - my) + (_2bx * q4 - _4bz * q2) * (_2bx * (q1q3 + q2q4) + _2bz * (0.5f - q2q2 - q3q3) - mz);
            s3 = -_2q1 * (2.0f * q2q4 - _2q1q3 - ax) + _2q4 * (2.0f * q1q2 + _2q3q4 - ay) - 4.0f * q3 * (1.0f - 2.0f * q2q2 - 2.0f * q3q3 - az) + (-_4bx * q3 - _2bz * q1) * (_2bx * (0.5f - q3q3 - q4q4) + _2bz * (q2q4 - q1q3) - mx) + (_2bx * q2 + _2bz * q4) * (_2bx * (q2q3 - q1q4) + _2bz * (q1q2 + q3q4) - my) + (_2bx * q1 - _4bz * q3) * (_2bx * (q1q3 + q2q4) + _2bz * (0.5f - q2q2 - q3q3) - mz);
            s4 = _2q2 * (2.0f * q2q4 - _2q1q3 - ax) + _2q3 * (2.0f * q1q2 + _2q3q4 - ay) + (-_4bx * q4 + _2bz * q2) * (_2bx * (0.5f - q3q3 - q4q4) + _2bz * (q2q4 - q1q3) - mx) + (-_2bx * q1 + _2bz * q3) * (_2bx * (q2q3 - q1q4) + _2bz * (q1q2 + q3q4) - my) + _2bx * q2 * (_2bx * (q1q3 + q2q4) + _2bz * (0.5f - q2q2 - q3q3) - mz);
            #ifdef FAST_INVERSE_SQRT
            	norm = fastInvSqrt(s1 * s1 + s2 * s2 + s3 * s3 + s4 * s4);
            #else
            	norm = sqrt(s1 * s1 + s2 * s2 + s3 * s3 + s4 * s4); // normalise step magnitude
            	if (norm == 0.0f) return; // handle NaN
            	norm = 1.0f/norm;
            #endif
            s1 *= norm;
            s2 *= norm;
            s3 *= norm;
            s4 *= norm;

            // Compute rate of change of quaternion
            qDot1 = 0.5f * (-q2 * gx - q3 * gy - q4 * gz) - beta * s1;
            qDot2 = 0.5f * (q1 * gx + q3 * gz - q4 * gy) - beta * s2;
            qDot3 = 0.5f * (q1 * gy - q2 * gz + q4 * gx) - beta * s3;
            qDot4 = 0.5f * (q1 * gz + q2 * gy - q3 * gx) - beta * s4;

            // Integrate to yield quaternion
            q1 += qDot1 * deltaTime;
            q2 += qDot2 * deltaTime;
            q3 += qDot3 * deltaTime;
            q4 += qDot4 * deltaTime;
            #ifdef FAST_INVERSE_SQRT
            	norm = fastInvSqrt(q1 * q1 + q2 * q2 + q3 * q3 + q4 * q4);
            #else
            	norm = sqrt(q1 * q1 + q2 * q2 + q3 * q3 + q4 * q4); // normalise quaternion
            	if (norm == 0.0f) return; // handle NaN
            	norm = 1.0f/norm;
            #endif
            q[0] = q1 * norm;
            q[1] = q2 * norm;
            q[2] = q3 * norm;
            q[3] = q4 * norm;

        }

#ifdef FAST_INVERSE_SQRT
float fastInvSqrt(float in) {
	unsigned int i = 0x5F1F1412 - (*(unsigned int*)&in >> 1);
	float tmp = *(float*)&i;
	return tmp * (1.69000231f - 0.714158168f * in * tmp * tmp);
}    
#endif
